...
class JassTokenTree {
    ...
    fun add(node: IJassNode?, token: Token?) {
    ...
    fun add(node: IJassNode?, token: TerminalNode?) {
        if (node == null || token == null) return
        add(node, token.symbol)
    }

    fun find(pos: Position?): IJassNode? {
    ...
    companion object {
        private class Holder(val node: IJassNode, val token: Token)
    }
}
